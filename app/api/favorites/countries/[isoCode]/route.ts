...
/**
 * The web half of the country-favorite surface's route handler (UYELIK-08 plan §5.3) — the
 ...
 * `/api/favorites/countries/{isoCode}` resource 1:1.
 */
export const dynamic = "force-dynamic";
export const fetchCache = "force-no-store";
export const runtime = "nodejs";

async function handlePut(request: Request, ctx: { params: Promise<{ isoCode: string }> }) {
  const { isoCode } = await ctx.params;
  const result = await handlePutFavorite(request, { kind: "country", isoCode });
