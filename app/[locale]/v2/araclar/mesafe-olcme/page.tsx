...
export async function generateMetadata({ params }: V2DistancePageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: "İki veya çok duraklı noktalar arasında WGS84 küresel elipsoid modeli ve Haversine formülü ile kuş uçuşu mesafe, uçuş süresi ve karayolu tahmini hesaplama.",
    alternates: {
    ...
    <div className="min-h-screen bg-background text-foreground pb-24">
      ...
      <JsonLd
        schema={learningResourceJsonLd({
          ...
          description: "İki nokta ya da çok duraklı güzergâh boyunca büyük daire jeodezik uzaklığını haritada ölçün.",
          ...
          <div className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-b from-card via-card to-muted/30 p-6 sm:p-10 shadow-lg">
            <div className="relative z-10 max-w-3xl space-y-4">
              ...
              <p className="text-muted-foreground text-sm sm:text-base leading-relaxed">
                Türkiye haritasında dilediğiniz noktaları işaretleyerek ya da 81 il merkezinden seçerek noktalar arası jeodezik mesafeyi, tahmini uçuş süresini ve karayolu farkını anında hesaplayın.
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mt-8">
              <div className="p-4 rounded-2xl bg-card border border-border shadow-2xs">
                <span className="font-heading text-2xl sm:text-3xl font-bold text-primary block">Haversine</span>
                <span className="text-xs text-muted-foreground font-medium">Büyük Daire Yayı Denklemi</span>
              </div>
              <div className="p-4 rounded-2xl bg-card border border-border shadow-2xs">
                <span className="font-heading text-2xl sm:text-3xl font-bold text-secondary block">6.371 km</span>
                <span className="text-xs text-muted-foreground font-medium">WGS84 Ortalama Dünya Yarıçapı</span>
              </div>
              <div className="p-4 rounded-2xl bg-card border border-border shadow-2xs">
                <span className="font-heading text-2xl sm:text-3xl font-bold text-accent block">800 km/s</span>
                <span className="text-xs text-muted-foreground font-medium">Seyir Hızı Uçuş Simülasyonu</span>
              </div>
              <div className="p-4 rounded-2xl bg-card border border-border shadow-2xs">
                <span className="font-heading text-2xl sm:text-3xl font-bold text-[var(--color-primary-dark,#7e3a1e)] block">%28 Eğim</span>
                <span className="text-xs text-muted-foreground font-medium">Topoğrafik Karayolu Katsayısı</span>
              </div>
            </div>
          </div>
        </div>
