...
export async function generateMetadata({ params }: V2AraclarPageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: "İnteraktif harita üzerinde kuş uçuşu jeodezik mesafe ölçümü, enlem/boylam koordinat tespiti ve çokgen alan hesabı.",
    alternates: {
          ...
          <div className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-b from-card via-card to-muted/30 p-6 sm:p-10 shadow-lg">
            <div className="relative z-10 max-w-3xl space-y-4">
              ...
              <p className="text-muted-foreground text-sm sm:text-base leading-relaxed">
                Harita üzerinde dilediğiniz noktaları işaretleyerek gerçek jeodezik mesafeyi, enlem/boylam koordinatlarını ve küresel çokgen yüzölçümünü WGS84 hassasiyetiyle anında hesaplayın.
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mt-8">
              <div className="p-4 rounded-2xl bg-card border border-border shadow-2xs">
                <span className="font-heading text-2xl sm:text-3xl font-bold text-primary block">WGS84</span>
                <span className="text-xs text-muted-foreground font-medium">Küresel Elipsoid Modeli</span>
              </div>
              <div className="p-4 rounded-2xl bg-card border border-border shadow-2xs">
                <span className="font-heading text-2xl sm:text-3xl font-bold text-secondary block">Haversine</span>
                <span className="text-xs text-muted-foreground font-medium">Büyük Daire Eğrilik Hesabı</span>
              </div>
              <div className="p-4 rounded-2xl bg-card border border-border shadow-2xs">
                <span className="font-heading text-2xl sm:text-3xl font-bold text-accent block">3 Birim</span>
                <span className="text-xs text-muted-foreground font-medium">km², Hektar, Dönüm Çıktısı</span>
              </div>
              <div className="p-4 rounded-2xl bg-card border border-border shadow-2xs">
                <span className="font-heading text-2xl sm:text-3xl font-bold text-[var(--color-primary-dark,#7e3a1e)] block">UTM + DMS</span>
                <span className="text-xs text-muted-foreground font-medium">Çift Projeksiyon Desteği</span>
              </div>
            </div>
          </div>
        </div>
