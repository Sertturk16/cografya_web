 ...
 *
 * EVERY BREAKPOINT IS ALSO SWEPT AT ITS OWN FLOOR: 640, 768, 1024, 1280 — `sm:`, `md:`,
 * `lg:`, `xl:`, the four this repo uses. A breakpoint-scoped layout is at its narrowest at
 * exactly the width that switches it on, so that width is the one place a row that only
 * "fits on wide screens" is guaranteed to be measured. The phone widths and 1440 cannot
 * stand in for it: a `sm:flex-row` toolbar is inactive at 390 and has 800px of slack at 1440.
 *
 * That is not hypothetical. `/dunya`'s catalogue toolbar went `sm:flex-row` at 640 with a
 * 654px row, scrolled the page sideways across [640, ~690) and passed every check the
 * sweep then ran (320/360/390/768/1440) — the 390–768 band this docblock had named as open.
 * 768 alone had been added earlier for the mirror case, `md:min-w-[900px] lg:min-w-0`,
 * inactive below 768 and harmless at 1440. 1024 and 1280 close the same hole for `lg:` and
 * `xl:`: a width that exceeds its breakpoint's floor but still fits 1440.
 */
