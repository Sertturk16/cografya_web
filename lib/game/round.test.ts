...
describe("the removed clock and snapshot (DEC 2026-07-30m/30n)", () => {
  ...
  it("keeps no timing field on the round state", () => {
    ...
    expect(Object.keys(state).sort()).toEqual([
      // Added by UYELIK-10 (§5.4): a stable, OPAQUE per-round id, generated once at
      // `createRound` and never regenerated by a transition — the idempotency key a save
      // submits. It is NOT the kind of state this guard exists to keep out: it carries no
      // elapsed time, is held only in memory for the lifetime of one round, and is never
      // itself persisted client-side — only sent to the server on an explicit save click.
      "clientRoundId",
      "currentWrongPicks",
...
describe("createRound's clientRoundId (UYELIK-10 plan §5.4 — the idempotent-retry mechanism)", () => {
  it("uses the injected generateId, defaulting to crypto.randomUUID()", () => {
    const state = createRound("provinces", POOL, noShuffle, () => "fixed-id-1");
    expect(state.clientRoundId).toBe("fixed-id-1");
  });

  it("defaults to a real, distinct id per round when no generator is injected", () => {
    const a = createRound("provinces", POOL, noShuffle);
    const b = createRound("provinces", POOL, noShuffle);
    expect(a.clientRoundId).toEqual(expect.any(String));
    expect(a.clientRoundId.length).toBeGreaterThan(0);
    expect(a.clientRoundId).not.toBe(b.clientRoundId);
  });

  it("stays IDENTICAL across every transition inside the same round — the whole no-duplicate-on-retry guarantee", () => {
    let state = createRound("provinces", POOL, noShuffle, () => "round-A");
    const id = state.clientRoundId;

    state = miss(state, 2);
    expect(state.clientRoundId).toBe(id);

    state = answerRound(state, target(state)).state;
    expect(state.clientRoundId).toBe(id);

    state = advanceRound(state);
    expect(state.clientRoundId).toBe(id);

    state = revealRound(state).state;
    expect(state.clientRoundId).toBe(id);
  ...
  it("gets a genuinely NEW id on a replay — a fresh createRound call, not a transition", () => {
    let idCounter = 0;
    const generateId = () => `round-${(idCounter += 1)}`;
    const first = createRound("provinces", POOL, noShuffle, generateId);
    const replay = createRound("provinces", POOL, noShuffle, generateId);
    expect(first.clientRoundId).toBe("round-1");
    expect(replay.clientRoundId).toBe("round-2");
    expect(replay.clientRoundId).not.toBe(first.clientRoundId);
  });
});
