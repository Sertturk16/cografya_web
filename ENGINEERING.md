...
| Review legs              | **§1** ground-truth order · **§2** stack lock · **§3** rendering per page type · **§4** SEO checklist · **§5** a11y floor · **§6** gates · **§10** do-NOT list — the repo truth a finding is scored against. §8 is the author's half; a leg reads it for context and never acts on it | Every review leg on a `cografya_web` PR, before assigning a severity; §3 whenever the diff touches rendering mode or route shape                                                                                                | `.claude/agents/pr-reviewer-high.md` `.claude/agents/pr-reviewer-medium.md` `.claude/agents/pr-validator.md` | `READ-ENG-WEB-REVIEWER` |
