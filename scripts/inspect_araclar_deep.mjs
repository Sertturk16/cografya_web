...
async function run() {
  ...
  const polyDashCheck = await p4.evaluate(() => {
    const btn = Array.from(document.querySelectorAll('button')).find(b => b.textContent.includes('Tuz Gölü'));
    if (btn) btn.click();
  ...
  await p4.screenshot({ path: path.join(ARTIFACTS_DIR, 'v2_araclar_dark_full.png'), fullPage: true });
  ...
  await pMobile.screenshot({ path: path.join(ARTIFACTS_DIR, 'v2_araclar_mobile_preset.png'), fullPage: true });
