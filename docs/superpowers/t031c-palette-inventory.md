...
## CLOSED 2026-09-19 — 939 → 15, and where the 15 are

Every row in this document is applied except the fifteen in the `v2-world-map-explorer.tsx`
table below, which are **left for T-031d** with their reason measured rather than asserted and
their line numbers re-verified against the tree at close. No product file outside that one
carries a raw palette class. `app/[locale]/(site)/turkiye/[slug]/page.tsx` is outside the count
and belongs to T-033.

The count is held by four arms, and none of them is a budget any more —
`components/ui/raw-palette-count.test.ts` (raw classes, bracketed colour values, colours inlined
outside a class; each a table of named files with exact counts and a reason, and zero everywhere
else) and `components/ui/compiled-stylesheet.test.ts` (the SHIPPED stylesheet). T-031d ends the
...
| Line          | Class(es)                                                                                                                                            | #   | Verdict  | Becomes                                                            | Note                                                                                                                                                                                                                                                                                                                                                    |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | --- | -------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 58            | `bg-amber-500/15 text-amber-800 dark:text-amber-200 border-amber-500/30`                                                                             | 4   | semantic | `bg-warning/15 text-warning-strong border-warning/30`              | `SpecialStatusBadge`, the shared component behind `dunya/[slug]:386`. Both move together or the flag changes colour between pages.                                                                                                                                                                                                                      |
| 670, 676, 685 | `stroke-sky-500/15`, `stroke-sky-400/40`, `stroke-sky-400/30`                                                                                        | 3   | data     | ~~a map-furniture token in the `--map-*` set~~ **LEFT FOR T-031d** | **APPLIED Task 9: not applied.** No member of the 17-token `--map-*` / `--province-*` / `--land-*` set is a graticule line; see the applied note below the table. The graticule: equator, tropics, prime meridian. Not a bridge token — this is the map's own surface, and `decoration` would delete the grid rather than re-bind it.                   |
| 690           | `fill-sky-400/50`                                                                                                                                    | 1   | data     | the same map-furniture token                                       | the graticule's text labels                                                                                                                                                                                                                                                                                                                             |
| 716           | `fill-slate-600/65 dark:fill-slate-700/70 stroke-slate-400/45 dark:stroke-slate-500/40`                                                              | 4   | data     | ~~`--land-inert` / `--province-stroke`~~ **LEFT FOR T-031d**       | **APPLIED Task 9: not applied.** `--land-inert` at this alpha reads 7.99 / 5.62 / 8.63 against the ocean where today's recede reads 1.68 / 1.27 / 1.75. A country with no entry — the "not published" fill. `--province-inert` and `--land-inert` already exist for exactly this on the Turkey map.                                                     |
| 725           | the same four plus `hover:fill-slate-500/75`                                                                                                         | 5   | data     | ~~as above~~ **LEFT FOR T-031d**                                   | **APPLIED Task 9: not applied**, as above. The filtered-out variant                                                                                                                                                                                                                                                                                     |
| 730           | `stroke-yellow-400 fill-yellow-500/90`                                                                                                               | 2   | data     | ~~the map hover/selected token~~ **LEFT FOR T-031d**               | **APPLIED Task 9: not applied** — the highlight has a second spelling neither arm counted (`floodColor` at `:650`), and fix round 1 bound THAT to `--primary` while these two classes wait. Hover and selection on the map surface; `--map-hover-width` and `--game-hover-edge` are the existing precedent. T-031d owns how this reads on the dark map. |
| 979, 981, 991 | `bg-amber-500/10 border-amber-500/30 text-amber-800 dark:text-amber-300`, `text-amber-600`, `border-amber-500/40 text-amber-800 dark:text-amber-200` | 8   | semantic | the `warning` family                                               | the continent-filter version of `v2-turkey-map-explorer:1032`, character for character. Same verdict, same target.                                                                                                                                                                                                                                      |

**Totals:** data 15, semantic 12, decoration 0.

**APPLIED, Task 9 — the 12 semantic only. THE 15 DATA ROWS ARE LEFT FOR T-031d.**

**Line numbers re-verified against the tree at branch close.** The `Line` column had drifted by
three as the semantic rows above them were applied, and this table is the record T-031d works
from, so a stale pointer here costs that task a search. The seven deferred spellings are at
`v2-world-map-explorer.tsx` **670, 676, 685, 690, 716, 725, 730**; `SpecialStatusBadge` is at 58
and the continent-filter banner at 979/981/991. `floodColor` at `:650`, cited below, is correct
as written.
...
| Line          | Class(es)                                                                                    | #   | Verdict    | Becomes                   | Note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------- | -------------------------------------------------------------------------------------------- | --- | ---------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 87            | `bg-amber-500/10 text-amber-600 border-amber-500/30`                                         | 3   | decoration | `Badge variant="default"` | the "Lider Tablosu" dialog badge, with a `Trophy` and its own word. **Applied as a variant swap, not a bare removal (Task 10):** this is a hand-drawn tinted badge beside a `variant="secondary"` one, so deleting the three classes leaves an untinted badge where a tinted one was and keeps a hand-drawn spelling in the census. The declared variant is the same shape — a 12% brand tint with the strong member as text — and measures 7.10 rest / 6.32 hovered against 2.96 light for the amber it replaces. |
...
| 144, 145 | `bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border-emerald-500/30`, `text-emerald-600` | 5   | semantic   | `Badge variant="success"` | the "Yayında" status badge with `CheckCircle2` — availability, one of the five. **Applied as the declared badge variant, not the hand-drawn tint (Task 10):** `components/ui/badge.tsx` already ships exactly this shape at a 15% tint, so hand-spelling a 10% one would add a second spelling of a badge the design system owns. The glyph inherits the badge's own text member rather than naming it again. Measured on the table row, which hovers to a 40% muted wash: 6.09 rest / 5.71 hovered light and 7.05 / 6.66 dark, against 4.87 / **4.54** for the emerald it replaces. |
...
| Line       | Class(es)                                                                                   | #   | Verdict    | Becomes                                               | Note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------- | ------------------------------------------------------------------------------------------- | --- | ---------- | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 76, 84, 92 | `bg-amber-500/10 text-amber-600 border-amber-500/30` and the emerald and orange equivalents | 9   | decoration | the one unlocked treatment the fourth already carries | three of the four achievement tints; the fourth is already `bg-primary/10 text-primary border-primary/30`, which is the tell. Unlocked versus locked is carried by a text `Badge` ("Kazanıldı" / "Kilitli"), a dashed border and `opacity-60`, so the per-achievement hue restates the title. **Read as "removed" means the HUE, not the tile (Task 10):** the field is constant once the three hues go, so it leaves the data and the unlocked branch names the treatment once. Glyph on its own 80% background plate over the tile, RE-PROBED on the real stack: **4.74 light / 5.31 dark**, against a model of 4.73 / 5.36 — they agree to 0.01 and 0.05, inside the ±1-byte band. (Task 10 recorded 4.49 / 4.63 here and generalised it into a rule that 80% is where `blendOver` and paint part. **Withdrawn:** that probe named a dark backdrop of `#212322`, a neutral grey, where this page's dark `--background` paints `#0b1416`; `ratio(#d0714e, #212322)` is 4.63 exactly, so the figure was the flat grey's own ratio and the probe was not standing on this stack. Direction was conservative and nothing shipped under a floor.) Against 2.99 / 3.39 / 3.35 light for the three it replaces -- one under the 3:1 graphical floor and all three under 4.5. The tile edge moves from 1.91 to 4.33 light. |
...
| 74, 78   | `text-emerald-600` ×2 | 2   | decoration | `text-primary`   | feature ticks on the first mode card; the `CheckCircle2` glyph and the feature sentence beside it carry the "included" meaning. **Bound, not deleted (Task 10):** card 1 is ALREADY `--primary` at its icon plate, its badge and its hover edge, so the tick was a second, raw spelling of an identity the card already carries in three declared places. 5.08 rest / 5.13 hovered light, 4.94 / 4.99 dark, against emerald-600's 3.62 / 4.61. |
| 122, 126 | `text-teal-600` ×2    | 2   | decoration | `text-secondary` | the same ticks on the second card — **in teal**. A tick that meant "yes" would not change hue between cards, so this is card variety — and card 2's variety is `--secondary`, in its plate, its badge and its hover edge. 5.84 / 5.89 light, 5.36 / 5.42 dark, against teal-600's 3.64 / 4.59.                                                                                                                                                 |
| 170, 174 | `text-cyan-600` ×2    | 2   | decoration | `text-accent`    | and cyan on the third, with the same `CheckCircle2` and the same feature sentence carrying it. Card 3 is `--accent` in its plate, its badge and its hover edge. 6.08 / 6.13 light, 5.38 / 5.44 dark, against cyan-600's 3.59 / 4.65.                                                                                                                                                                                                           |
| 205      | `text-purple-600`     | 1   | decoration | `text-primary`   | a `Brain` glyph beside its heading — and the OTHER glyph in the same two-item strip is already `text-primary`, so this is a straggler in a set of two. On the strip's 40% muted wash: 4.61 light (from 4.98) and 4.57 dark (from 2.82, which was under the floor).                                                                                                                                                                             |
...
| Line          | Class(es)                                            | #   | Verdict    | Becomes        | Note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------- | ---------------------------------------------------- | --- | ---------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 296, 306, 316 | `text-sky-500`, `text-emerald-500`, `text-amber-500` | 3   | decoration | `text-primary` | one glyph per favourite entity type — and every row already renders a text `badge` ("Ülke" / "Bölge" / "Kıta") plus a subtitle. Note the emerald "Bölge" glyph is **not** a region identity: all seven regions share it, so it must not be bound to `--region-*`. **Bound to `--primary`, not deleted (Task 10):** the `province` case of the same switch is ALREADY `text-primary`, so three of four entity glyphs were raw and the fourth was brand. Binding to the brand token is also what keeps the emerald glyph out of `--region-*`, which this note forbids. On the row's 60% muted plate they measured 2.43 / 2.22 / 1.92 light, all far under the 3:1 graphical floor; `--primary` measures 4.61 light and 4.57 dark. |
| 437, 455, 464 | `text-rose-500`, `text-amber-500`, `text-indigo-500` | 3   | decoration | `text-primary` | `Heart` / `Trophy` / `Ruler` on labelled stat rows — and the FOURTH tile in the same row of four is already `text-primary`. **Bound, not deleted (Task 10)**, same as the row above. On the tiles' 60% background wash: 3.64 / 2.07 / 4.43 light before, 4.97 after, matching the tile that was already brand.                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 983, 985      | `text-emerald-500`, `text-sky-500`                   | 2   | decoration | `text-primary` | the measurement-type glyphs, whose THIRD arm (`Ruler`, distance) is already `text-primary`. **Bound, not deleted (Task 10).** 2.22 / 2.43 light on the 60% muted plate before, 4.61 after.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Line       | Class(es)                                                                                       | #   | Verdict    | Becomes                                          | Note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ---------- | ----------------------------------------------------------------------------------------------- | --- | ---------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 17, 34, 51 | `from-amber-600 to-amber-800`, `from-emerald-700 to-emerald-900`, `from-blue-700 to-indigo-900` | 6   | decoration | one shared `from-primary to-primary-strong` wash | three learning-path header washes. Per-card variety: the paths are named in the header they sit behind, and nothing else paints them. **"Removed" cannot mean deleted here (Task 10):** the header is a white-on-colour plate, so a gradient with no stops leaves white text on no background. The three collapse into ONE brand wash, and the plate's own white utilities become the primary foreground so both themes pair correctly. Measured at the from-end, where the badge and glyph sit: 5.13 light / 4.94 dark, against 3.20 for the amber card. The badge goes solid (5.13 / 4.94, from 2.54) and the eyebrow drops its 80% alpha (5.13 / 4.94, from 2.57) — both were failing on the amber card before. **Painted afterwards, and the paint corrected the plate (Task 10):** `app/globals.css`'s base layer sets `h3, h4 { color: var(--foreground) }`, so the header's title does NOT inherit the plate's colour the way its spans do — with the explicit class removed it painted 2.88 light and 2.92 dark on the wash. The `<h3>` carries `text-primary-foreground` itself. Painted: title and eyebrow **5.13** light / 5.01 dark at the from-end and 7.03 / 7.05 at the to-end; badge 5.13 / 4.94; play glyph 4.37 / 4.64 on its 20% plate. **Two corrections at branch close, both re-painted.** The from-end figure is 5.13, not the 5.20 recorded in Task 10: 5.13 is the exact `--primary-foreground` on `--primary` pair — the same pair the badge carries, which is why both read 5.13 — and a sample taken a few pixels into a `to-br` gradient reads a little higher because the gradient has already advanced. And the PRE-FIX figure was recorded only at the from-end (2.88 / 2.92); at the TO-end, where the title actually sits, the base layer's `--foreground` measured **2.13 light / 2.07 dark**, which is the worse half and was missing from the record. |
...
| Line       | Class(es)                                             | #   | Verdict    | Becomes        | Note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------- | ----------------------------------------------------- | --- | ---------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 14, 20, 32 | `text-amber-500`, `text-emerald-500`, `text-rose-500` | 3   | decoration | `text-primary` | `Star` / `Trophy` / `Video` on benefit rows, each with its own title and description. **Bound, not deleted (Task 10):** the FOURTH benefit row in the same array is already `text-primary`, so removing the three leaves one tinted glyph in a column of four. On the row's own card plate they measured 2.13 / 2.47 / 3.75 light, all under the 3:1 graphical floor or within a rounding of it; `--primary` measures 5.13 / 4.99. **PAINTED at branch close, and the label was the only thing wrong:** this plate renders on `/kayit` to a logged-out visitor, so it never needed the session Task 10's report assumed. Painted on the real page it reads **5.13 light / 4.99 dark**, the recorded figures exactly. |
| 90         | `text-emerald-500`                                    | 1   | decoration | removed        | `ShieldCheck` in the trust footer — a singleton beside `text-foreground` prose, not a member of the benefits set, so it inherits rather than binds. 2.34 light on the footer's 30% muted wash, 14.15 after.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Line | Class(es)      | #   | Verdict    | Becomes                                                           | Note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ---- | -------------- | --- | ---------- | ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 652  | `bg-teal-500`  | 1   | decoration | `bg-muted-foreground/40`, the bullet the third state already uses | the coastal marker dot. It is `aria-hidden` and the word "Kıyı" is its sibling `<span>`. **Applied as the neutral bullet, not a deletion (Task 10):** deleting the fill leaves an invisible 8px box, so the two states take the value the ternary's THIRD arm already shipped. All three arms then differ only in their label, so they collapse into one.                                                                                                                                                                                                                                                                                                                               |
| 660  | `bg-amber-500` | 1   | decoration | `bg-muted-foreground/40`, as above                                | the inland dot, labelled "İç". Two values with two colours, which would argue `data` — but the **third** state on the same ternary is already `bg-muted-foreground/40`, so this is not a data set, it is a bullet in front of a word. Measured as a decorative mark, not a graphical object: 1.95 light / 2.36 dark on the row (**re-painted at branch close and confirmed**, against 2.34 in Task 10's report, which had labelled the same pair `rest / hovered` rather than `light / dark`; painted, the hovered pair is 1.93 / 2.33), where teal-500 was 2.42 and amber-500 2.13 — all three under 3:1, and `aria-hidden` with the word beside it is why none of them is held to it. |
...
Three findings the closing review surfaced that are outside this branch's count and outside its
scope. Each is named here so the next task inherits a fact rather than a surprise.

1. **`badgeVariant` in `components/v2/v2-learning-paths.tsx` is dead**, by exactly the test that
   deleted `topics[].color` from `v2-study-strategy-guide`: the three entries declare
   `badgeVariant: "primary" / "secondary" / "outline"`, and the header `Badge` takes a
   hand-written `className` (`bg-primary-foreground text-primary border-transparent`) and never
   reads the field. Pre-existing, carries no palette class, and outside the count — but the
   deadness test was applied to one object in this task and not to its neighbour. Deleting it is
   a three-line change for whoever next touches that file.

2. **Sixteen `<h4>`s in `components/v2/v2-tool-educational-content.tsx` do not take their row's
   hue.** Each sits in a `flex items-center gap-2 text-primary|secondary|accent` row beside a
   glyph, and `app/globals.css`'s base layer sets `h3, h4 { color: var(--foreground) }`, so only
   the glyph moves. Three sibling files already solved this shape with `[&_hN]:text-inherit`
   (`v2-turkey-regions.tsx`, `v2-turkey-map-explorer.tsx`, `v2-world-map-explorer.tsx`). **No
   contrast risk** — the headings read 14.97 light / 14.73 dark on `--card` — so this is a paint
   question, not a model one. It is recorded because commit `83b43d9` re-spelled two of those
   rows for a heading that cannot take the hue, which is a rotation applied to a mark that does
   not rotate.

3. **`components/v2/v2-header.tsx:140` carries `dark:text-[#e2896a]`**, one bracketed literal on
   the wordmark and the only entry in `ARBITRARY_PINNED` that is not a map surface. It lightens
   the mark over the dark `--primary` (`oklch(0.65 0.13 40.65)`). Rebinding it repaints a brand
   mark in one theme and needs its own measurement, so it is named rather than absorbed.
