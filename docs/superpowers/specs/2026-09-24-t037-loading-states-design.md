...
**Status:** implemented on `feature/t037-loading-states`; loading.tsx rule amended during
execution — see Decisions. Owner decision recorded on 2026-09-24: the widest streaming scope —
`loading.tsx` plus a Suspense boundary at **every** section that consumes server-fetched data, not
only the five heavy surfaces.
...
| Which routes get a `loading.tsx`?                   | Amended during execution (task-14-addendum.md, Ruling A): a rendered `loading.tsx` commits the response to 200 the instant it renders, so a later `notFound()` or `redirect()` can only become a `<meta>` tag — and this repo's hard rule "unknown slug → real 404" outranks the original plan below. A route gets a `loading.tsx` only when it is `force-dynamic` AND its page decides neither `notFound()` nor `redirect()` before its first return. Three routes today (`/turkiye`, `/dunya`, `/kayit`); the `[slug]` details and the account pages deliberately have none and keep their real 404/307. |
...
| What happens when a fetch throws inside a boundary? | Unchanged from today: the error reaches `(site)/error.tsx`. No per-section error boundaries in this task; `*Resilient` re-throwing at runtime is a recorded decision that keeps ISR serving the last good page, and a boundary that swallows it would change that.                                                                                                                                                                                                                                                                                                                                         |
...
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `hub`     | `PageContainer` → `BreadcrumbsSkeleton` → `HeroSkeleton tier="hub" tiles={4}` → `PlateSkeleton aspect="map"`                                                          | `/deprem`, `/deniz`, `/turkiye`, `/dunya`, `/kitaplar`, `/turkiye/bolge`, `/oyun/bolge-bolge-il`, the four basins, the three tools |
| `detail`  | Band `section` → `PageContainer space="band"` → breadcrumbs, `HeroSkeleton tier="detail" tiles={4}`; then `PageContainer` → 12-col grid (8 + 4) of `CardGridSkeleton` | `/turkiye/[slug]`, `/turkiye/bolge/[slug]`, `/dunya/[slug]`, `/dunya/kita/[slug]`, `/kitaplar/[slug]`                              |
| `account` | `PageContainer space="tight"` → breadcrumbs → heading + lede bars → `CardGridSkeleton columns="2" count={4}`                                                          | `/hesabim`, `/hesabim/ayarlar`                                                                                                     |
...
specimen showing the `hub` and `detail` shapes at panel width, and the `components/patterns/`
...
| `/turkiye` (dynamic)    | —        | Two hero tiles (İl sayısı, İlçe sayısı); explorer + `itemListJsonLd` + `collectionPageJsonLd` (description uses the count).                                                                                                                                                                             | **L** `hub` |
| `/dunya` (dynamic)      | —        | One hero tile; explorer (+ `V2WorldContinents` in its `middleSections`) + both JSON-LD.                                                                                                                                                                                                                 | **L** `hub` |
...
Totals after the split: **3 `loading.tsx`** (`turkiye`, `dunya`, `kayit` — Ruling A, task-14-addendum.md,
amended this down from the plan above's nine) and roughly **35 Suspense boundaries** across 30
pages. The `play` shape is used only as a Suspense fallback.
...
4. **`lib/api/cached-loaders.test.ts`** — Section 2's dedupe assertion.
...
- Seeing the skeletons: with the dev container serving `:3000`, `docker pause cografya-api-dev`
  for the duration of one screenshot round, navigate client-side to `/turkiye`, `/dunya`,
  `/hesabim`, `/turkiye/ankara`, `/oyun/81-il`; then `docker unpause`. Screens at 320, 360, 390
  and 1440 px, light and dark. This is the only local way to hold a fallback on screen long enough
  to measure; it is not a production procedure.
