...
**Status:** designed, awaiting owner review. Owner decision recorded on 2026-09-24: the widest
streaming scope — `loading.tsx` plus a Suspense boundary at **every** section that consumes
server-fetched data, not only the five heavy surfaces.
...
| Which routes get a `loading.tsx`?                   | Every route whose page still awaits a fetch before returning JSX after the split (the `[slug]` details, which must resolve the entity for `notFound()`, and the auth pages, which must resolve the session for `redirect()`), plus the five dynamic routes. A page that returns JSX synchronously and streams all its data does not need one. Section 4 lists each.         |
...
| `/hesabim` (dynamic)         | `getSession` → `redirect`; `readProfileForPage`         | `V2MemberHub` behind an async loader for the four reference lists.                       | **L** `account` |
| `/hesabim/ayarlar` (dynamic) | `readProfileForPage` → `redirect` / `unavailable` alert | `V2AccountSettings` behind an async loader for provinces.                                | **L** `account` |
