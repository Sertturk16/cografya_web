...
| **Licence**                | **Public Domain** — verbatim: _"All versions of Natural Earth raster + vector map data found on this website are in the public domain. You may use the maps in any manner…"_ and _"No permission is needed to use Natural Earth. Crediting the authors is unnecessary."_ (Natural Earth Terms of Use). Attribution NOT required. _(Corrected 2026-08-30, turkiye-yenileme PR-B: the previous "no restrictions … may use in any manner" quotation does not appear on the licence page — measured `provenance/datasets.md` line 51 `AÇIK 1`, `grep -ic "no restrictions"` → 0; the licence conclusion is unchanged, only the quotation is.)_ |
...
### Second consumer (2026-08-30, `turkiye-yenileme` PR-B)

This snapshot now feeds a SECOND generator, `scripts/generate-tr-context.mjs`, which
re-projects the same committed bytes into the pinned TR context frame
(`scripts/lib/tr-frame.mjs` `TR_CONTEXT_FRAME`) rather than the Natural-Earth-1 world frame
