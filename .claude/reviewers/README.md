...
Atlas drives the fan-out through the **`review-pr` skill**, which lives in the
**orchestrator home, not this repo** — `<orchestrator-home>/.claude/skills/review-pr/SKILL.md`
(sibling of `cografya_web/`). It landed and made its first live run on PR #16.
...
(→ DEC 2026-07-11 / 2026-07-12 / 2026-07-18e; the shape rules themselves are **binding and
live in `CONVENTIONS.md` §2**, which wins over this table on any conflict — this table only
restates them). Atlas evaluates
...
| **FENER** (SEO)      | `sonnet` | Spawned as `subagent_type: cografya-seo` (**not** from a file here). When the diff touches the SEO surface — metadata/`buildMetadata`, routing/slugs, canonical/hreflang, JSON-LD, `sitemap.ts`/`robots.ts`/`llms.txt`, or data-fetching/schema. **Also, independently (→ DEC 2026-07-18e):** (a) any **rendering-mode change** — SSG→SSR/CSR, or wrapping content in `dynamic(…, { ssr: false })` — which is **never presentation-only**; (b) any **CWV-relevant** diff — embeds, image sizing, fonts, or layout-shift-prone styling — even when nothing else applies. **Skipped on a presentation-only diff** (JSX/markup, copy/i18n values, styling, assets) only when none of the above is touched |
| **a11y-reviewer**    | `sonnet` | When the diff touches **rendered output** — JSX/markup, components, styling, templates, or anything altering the served DOM. **SKIPPED** when the diff is only metadata/config/build/test/docs with no rendered-output change (→ DEC 2026-07-18e)                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
