...
// Deliberately NOT `@/i18n/navigation`'s `useRouter` — the same reasoning the V1 login form
// records: `router.replace()` below receives the BFF's own `safeReturnPath()` output, a final
// path with its locale prefix already resolved.
...
/**
 * `?returnTo=` off the CURRENT url — the identical `useSyncExternalStore` idiom the login
 * screen uses. `verify-email` is the second token-issuing action, so it needs the same
 * mechanism.
 */
const NEVER_CHANGES = () => () => {};
...
/**
 * 60 s — the api's own resend cooldown window (`cografya_api` `src/auth/auth.types.ts`,
 * `limit: 1, windowMs: 60_000`). NOT load-bearing: the api answers the same 202 whether it
 * sent anything or not, so a drifted number only changes when the button re-enables, never
 * what happens. Recorded as a UI affordance, not a mirrored constant, and it gets no gate.
 */
...
/**
 * `/e-posta-dogrulama` · `/en/verify-email`. V2 port of
 * `components/auth/verify-email-form.tsx`; behaviour is unchanged.
 *
 * A reload here always lands on the empty form — no PII is kept in browser storage — so the
 * address is re-typed rather than remembered.
 */
export function V2VerifyEmailCard({ locale }: { readonly locale: Locale }) {
  ...
  // never a claim about when the api will actually accept another resend.
  useEffect(() => {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await submitAuth(
      "verify-email",
      { email, code },
      // Same `||` (not `??`) reasoning as the login form: an empty `?returnTo=` parses to
      // `""`, which a nullish check would let through unfixed.
  ...
  return (
    <form className="space-y-5" onSubmit={(event) => void handleSubmit(event)} noValidate>
      ...
      <FormField
        ...
        // Never `type="number"`: the api mints the code with `padStart(6,'0')`, so `'000000'`
        // is a valid, real code and a leading zero must never be silently stripped.
        type="text"
        inputMode="numeric"
        pattern="[0-9]{6}"
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Button type="submit" variant="primary" isLoading={submitting} leftIcon={<MailCheck />}>
          {t("verify.submit")}
        </Button>
        <Button
          type="button"
      ...
      {/* Permanently mounted, text-only update — the reliable live-region pattern: empty text
          until the outcome is known, never a node whose role AND final content both enter the
          DOM in the same render. */}
